...
export async function POST(req:NextRequest) {
 ...
 const {data:intro,error}=await db.from('daily_intros').select('*').eq('id',body.introId).single()
 if(error || !intro || !await authenticatedProfileOwner(req.headers,intro.user_id))return NextResponse.json({error:'Introduction not found'},{status:404})
