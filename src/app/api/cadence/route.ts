...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const match = await saveMatch({
        ...
        expansion_points: candidateComposite?.interest_tags
          ?.filter((t: string) => !userComposite?.interest_tags?.includes(t))
          ?.slice(0, 5) || [],
    ...
    if (introsGenerated === 0) {
      await updatePoolState(userId, 'empty_pool', 'no_compatible_users')
      return NextResponse.json({
        ok: true,
        introsGenerated: 0,
        poolState: 'empty_pool',
        message: "We're finding people for you. Check back soon.",
      })
    }

    return NextResponse.json({ ok: true, introsGenerated })
  } catch (err) {
    console.error('Route error:', err)
    const message = err instanceof Error ? err.message :
      (typeof err === 'object' && err !== null && 'message' in err) ? String((err as Record<string, unknown>).message) :
