...
> **§4 REPLACED (2026-08-24, second pass):** my "logged only" downgrade was itself wrong and is withdrawn — see `specs/matching-v2-content-targeting.md`. Logged-but-unrandomised is no experiment at any N. The factor **is randomised from day one**, with the aiming variable changed from reader asset-preference to **pair similarity** (Montoya F2, r ≈ .47 in no-interaction settings), and the readout gated on event count rather than a calendar window.
