...
**F4. ⚠️ CORRECTED 2026-08-24 — personality-matched messaging does NOT move clicks; it moves considered decisions, and not at our scale.**
> **The original F4 was wrong and it was load-bearing.** It read: "Matz, Kosinski et al. (PNAS 2017): 3.5M people, real field experiments — ads framed to the recipient's extraversion or openness produced up to 40% more clicks and 50% more purchases," and concluded that (a) the reader instrument should be a Big Five short form because E and O are the proven targeting axes, (b) coarse measurement suffices, and (c) "expected effects are big enough to detect at our scale." **All three conclusions are unsupported.** Verified against the paper's own text (`specs/research-pitch-demand-findings.md`, and re-checked independently 2026-08-24):

- **Study 1** — the largest, N = 3,129,993: *"There was no significant interaction effect on clicks (χ²(1) = <0.001; OR = 1.0 [0.96–1.04], p = 0.98)."* Purchases **were** affected (OR = 1.54 [1.25–1.90], p < 0.001).
- **Study 2** — N = 84,176, i.e. 37× smaller: clicks OR = 1.38, installs OR = 1.31. This is where "40% more clicks" comes from.
- **Study 3** — N = 534,250: tailored ad set vs **standard/generic** ad set (clicks OR = 1.30). That is a *craft* comparison, not a targeting comparison.
- Users were not individually randomised in the field studies (Facebook's optimiser chose exposure) — the Eckles/Gordon/Johnson internal-validity critique.

→ **So for PLY, two things.** (1) **A 🔥 fire is a click**, and the click effect at 3.1M was exactly zero. Targeting cannot be justified on fire rate. (2) The *purchase* effect was real, and a purchase is a considered, costly decision — which in our funnel maps to the photo-reveal decision and the date, not the fire. So the honest statement is **not** "targeting doesn't work"; it is **"targeting does not move clicks, may move considered downstream decisions, and we can measure neither at N≈50."** The operational conclusion is unchanged either way: craft first, aim later. The umbrella meta-analysis puts PLY's exact configuration — traits inferred from proxies, matched message vs a *good* generic — at r ≈ .14–.17, and PLY's minimum detectable reader×content interaction at 1,200 events is OR ≈ 2.24 against a literature-calibrated OR ≈ 1.67. Below the floor.
