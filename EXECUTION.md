...
**Final release verification, 2026-09-10:** Runtime `81f99a681b96485f1a98e30bd71fd4db1943fa23` is live on production after staging `c3818105b37536f9d3b5994e355bad12331e2f4d`. Staging CI [34541723210](https://github.com/charlesrogers/people-like-you/actions/runs/34541723210) and production CI [34542295870](https://github.com/charlesrogers/people-like-you/actions/runs/34542295870) succeeded. Both public prompt endpoints serve `stories-v1.3-heic`; the real HEIC worker in each deployed container converted the non-personal fixture to JPEG (724 bytes). Production health is 200; unauthenticated photo management is 401. Private capture status remains enabled with four captured questions, no processed transcripts or approved examples yet, and training eligibility zero. No existing member photos or recordings were replaced or removed. Final main suite: 192 passing tests; staging suite: 211 including unrelated staging tests. Web production and native simulator builds pass. Native edits remain in the separate checkout; no TestFlight upload. This verifies deployment and fixture behavior, not a completed real-iPhone retest or beta dataset.

**HEIC follow-up, 2026-09-10 — LIVE:** Charles identified the failed photo as likely HEIC from his iPhone gallery. Added explicit HEIC/HEIF-to-JPEG conversion before moderation/storage, with bounded worker-thread execution, a 25 MB input cap, and browser fallback to server conversion. Generated non-personal 64×64 HEIC fixture verifies actual decoding and the complete upload route (moderation and storage receive JPEG; corrupt HEIC is rejected before either). Full suite now 192 tests; production build passes. Dependency `heic-convert` runs in the server worker; Docker copies complete node_modules. No new audit findings in its dependency chain. Original failure remains unproven; former UI swallowed the returned error. Native photo selection already converts UIImage to JPEG and now resizes it.

**2026-09-10 — Phone onboarding feedback fixes (LIVE):** Four story groups now explicitly ask for ONE answer each, explain the group purpose, and show full questions beneath labels. Removed the full eight-step header and undisclosed 90-second recorder stop; complete audio duration is decoded beyond five minutes. Photos are optional even after an upload error; camera files resize to JPEG, errors retain server detail, touch removal is visible, and Profile has add/remove photos backed by owned `/api/photos`. Successful uploads are not repeated on retry. Pending synthesis no longer falsely says saved stories do not exist. Existing recordings are unchanged. Confirmed four recent Charles audio objects are in storage (84/90/45/26 seconds), pending processing; no recent photo reached moderation/storage. Actual photo failure cause remains unconfirmed because old UI discarded the error. Storage upload/cleanup probe passed. 192 tests, TypeScript, production build and native simulator build pass; lint has nine baseline errors. Mobile-width fixture walkthrough verified resize (4200→1800 px), removal, detailed upload failure and continuing without photos. Browser file chooser was extension-blocked, so fixture conversion was exercised through a generated local image instead. Native copy/optional photos prepared, no TestFlight upload.
...
- **Organic cluster v2 — WAITLIST FIRST + positioning (2026-09-02, LIVE IN PROD)**: after two rounds of Charles feedback, every organic page now opens with the brand-yellow hero + the real capture form above the fold (shared `WaitlistForm` extracted from `WaitlistCapture`, root page unchanged; `IntlWaitlistForm` restyled to match; per-page `source` tags like `seo-metro-provo-orem`). Copy rules now locked in tasks/lessons.md: pages say plainly PLY **is** an LDS dating site (never "whatever their faith", no Mormon-terminology lecturing — ad landing `/` still community-silent per launch-plan §5); "challenges" never "problems" (en/es/pt); no editorializing about a community's self-regard; every metro page sells reach — "close enough to actually date" incl. adjacent-metro unlock (Philly→NYC/DC) and family-ward singles (shared FAQ). Also from review: intl waitlist dedupe keys namespaced `CC:digits` (a 10-digit Nigerian mobile collides with the US keyspace otherwise) and intl rows excluded from the US no-metro position cohort.
