...
- **T-SIM PASSED — matching v2 design is statistically sound (2026-08-22)**: `scripts/matching_v2_sim.py` (venv per `scripts/requirements-sim.txt`; python 3.13, no app deps). All four §4 gates green over 2,000 simulated cohorts each — results `specs/matching-v2-tsim-results.md`, raw run log `specs/matching-v2-tsim-run.txt`. **S2, the C12 gate, passes decisively**: injecting a +5pp position-3 drift moves the angle estimates by ≤0.044pp (measured as bias-with-drift minus bias-without-drift, so estimator shrinkage can't be mistaken for confound leakage). Randomized angle order + an explicit position term do decorrelate the confound — **do not redesign the sequencing scheme**. Three things must be carried forward before launch:
  1. **The §11 fitter recipe is binding** (results §3). statsmodels' two GLMM fitters fail in opposite directions. `matching_v2_analysis.py` must take **point estimates from `fit_vb`**, **SEs/CIs/contrasts from `fit_map`**, and **reader-level covariates (`pickiness`) from a reader-cluster-robust GLM**. Fitting with `fit_map` alone understates position-3 drift by −2.5pp and manufactures a −1.79pp drift under the pure null (8.6% FPR); `fit_vb` alone gives 80% CI coverage instead of 95%.
  2. **Realized MDEs for the E-STYLE registration** (no invented thresholds): angle contrast **+10.62 / +7.46 / +5.11 pp** at 1,200 / 2,250 / 4,500 events; content lead **+7.58 / +5.37 / +3.70 pp**; position-3 **+12.50 / +8.75 / +5.96 pp**. Validated empirically (injected +7.46pp → 78.0% realized power vs 80% target).
  3. **E-STYLE hypotheses need revision** (results §5). **H1/H2 are untestable at launch size** — a reader-segment × angle interaction has a measured MDE of **+16.66pp** at 2,250 events (2.2× the main effect); reclassify as exploratory, powered at ≥9,000 events. **H4 is underpowered** across most of its predicted +2–8pp range (MDE +8.75pp at 2,250). H3 is the best-powered hypothesis. Also: testing all six pairwise angle contrasts uncorrected has a **23.6% family-wise false-positive rate** — pre-register the specific contrasts or gate on the joint 3-df Wald test first.
