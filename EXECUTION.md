...
**HEIC follow-up, 2026-09-10:** Charles identified the failed photo as likely HEIC from his iPhone gallery. Added explicit HEIC/HEIF-to-JPEG conversion before moderation/storage, with bounded worker-thread execution, a 25 MB input cap, and browser fallback to server conversion. Generated non-personal 64×64 HEIC fixture verifies actual decoding and the complete upload route (moderation and storage receive JPEG; corrupt HEIC is rejected before either). Full suite now 192 tests; production build passes. Dependency `heic-convert` runs in the server worker; Docker copies complete node_modules. No new audit findings in its dependency chain. Original failure remains unproven; former UI swallowed the returned error. Native photo selection already converts UIImage to JPEG and now resizes it.

**2026-09-10 — Phone onboarding feedback fixes (release verification pending):** Four story groups now explicitly ask for ONE answer each, explain the group purpose, and show full questions beneath labels. Removed the full eight-step header and undisclosed 90-second recorder stop; complete audio duration is decoded beyond five minutes. Photos are optional even after an upload error; camera files resize to JPEG, errors retain server detail, touch removal is visible, and Profile has add/remove photos backed by owned `/api/photos`. Successful uploads are not repeated on retry. Pending synthesis no longer falsely says saved stories do not exist. Existing recordings are unchanged. Confirmed four recent Charles audio objects are in storage (84/90/45/26 seconds), pending processing; no recent photo reached moderation/storage. Actual photo failure cause remains unconfirmed because old UI discarded the error. Storage upload/cleanup probe passed. 186 tests, TypeScript, production build and native simulator build pass; lint has nine baseline errors. Mobile-width fixture walkthrough verified resize (4200→1800 px), removal, detailed upload failure and continuing without photos. Browser file chooser was extension-blocked, so fixture conversion was exercised through a generated local image instead. Native copy/optional photos prepared, no TestFlight upload.

**2026-09-10 — Operational model-data capture LIVE, enabled:** Production runtime `488b660` serves `capture-v1.1`; migrations `025_model_data_capture.sql` and `026_model_data_source_erasure.sql` are applied to the shared database. Private status verified `captureEnabled=true` at approximately 21:45 UTC. Captures immutable questions/transcripts, analysis, synthesis, pitch packets, every draft/critic/regeneration, exact revisions and linked feedback. All records are constrained training-ineligible. Charles explicitly deferred human review and training opt-in: no review UI, cohorts, exporter, model download/training or offline generation was built. Existing intake stays four >=20-second recordings; follow-ups remain V2. Initial capture inventory is empty, not an approved beta dataset. 180 tests (27 capture fixtures), TypeScript, web build and native simulator build pass; lint retains nine baseline errors, none new. Native capture edits are prepared in the separate checkout, not uploaded to TestFlight. Full paths, limitations, deployment verification and rollback instructions: `specs/model-data-capture-v1-operations.md`. Turn capture OFF before rolling back to old application code.

**2026-09-10 — disk-full outage recovered during release:** Shared Postgres could not start on a 100%-full 150 GB root disk, causing both sites to return 503 before the onboarding deployment. Approximately 31 GB of host system logs, 20.2 GB of Coolify application logs, and 37 GB of database backup files were major consumers. Preserved/compressed the 19 GB archived syslog, rotated the current 11.7 GB log, and set daily/100 MB rotation with hourly checks and seven compressed archives. Approximately 20 GB became available; Postgres and both sites recovered. No app data or database backups were deleted. Remaining: Coolify log retention, malformed Docker-log retry loop, and backup/diagnostic-container audit. See `specs/server-disk-recovery-2026-09-10.md`.

**2026-09-10 — V1 recording intake (LIVE on production):** Charles deferred personalized follow-ups to v2. Web onboarding now requires four saved recordings of at least 20 seconds each; the recorder displays the minimum, elapsed/remaining time, and a save gate with playback/retry. Cancel no longer shares the upload path. Server decodes audio via FFmpeg before storage/replacement and returns measured duration; resume excludes replaced/short takes. New `/api/onboarding-prompts` shares bank content, minimum, and count with native. Runtime image and staging test runner install FFmpeg. Native changes in the separate local `PeopleLikeYou` project compile for iOS Simulator; no TestFlight upload yet, per wife-on-web-first sequence. Local verification: 153 tests, production build, recorder browser walkthrough (short/cancel/20s/no-autostop/retry). Spec: `specs/onboarding-recordings-v1.md`. Release `35ae0ca`, staging cherry-pick `387378e`; staging run 34444106005 and production run 34519017208 green. Both live endpoints report `recordings-v1-20s` / 4 / 20. Production served JS contains the new minimum/progress/save copy; a one-second generated clip claiming 90 seconds was rejected 422 with measured duration 1 before storage. Production health database/auth/storage all OK. The new flow is ready for Charles and his wife; real-device onboarding and TestFlight remain next.
...
- **T16c**: waitlist reworked to **phone + ZIP only** per Charles. Migration 022 (email nullable, `phone_normalized` unique, `zip3`/`city`/`state`). `/api/waitlist` keys on phone. New `src/components/WaitlistCapture.tsx` = capture form + success **popup** naming their city ("We're launching in Provo, UT soon"), queue position, and a **copy-paste invite message** carrying their referral link. **Root `/` is now the waitlist**; old marketing homepage moved to **`/welcome`**; `/waitlist` still serves the same component (existing Meta ads point there). Referral reward per Charles = **early access + extra matches for a set number of weeks** (no discount code — PLY has no payments); copy is aligned to the approved T19 boost (+1 intro/day ×7d per invite).

- **T16d (2026-08-14, LIVE IN PROD)**: waitlist copy rewritten with Charles over several passes → headline "Matches won't get you married. You need a spark."; body leads on the tailored-preview promise and closes on "a sea 🌊 of swipes". Value-prop bullets and the "Add your number and ZIP" line were deliberately cut — the page is now logo / headline / body / form, fitting above the fold. **Community is never named on the page** (launch-plan §5 compliance: Meta bans ads implying a viewer's religion, landing page must match the ad, dating authorization still pending). Waitlist **rate limit raised 8 → 40/min** with a friendlier 429 — group signups share one NAT'd IP, so at 8/min the ninth person at an event was blocked. Phone handling verified by a 21-case suite (formats, rejects, dedupe across formatting) plus keystroke-level formatter checks.

- **Waitlist Discord digest (2026-08-17, LIVE)**: `GET /api/cron/waitlist-digest` posts every new signup (phone, ZIP, city/metro, referral attribution) plus a per-metro tally to Discord. Server cron `0 14 * * *` (8am MT) runs `/root/ply-waitlist-digest.sh`, which **checks the HTTP status and alerts Discord on failure** — unlike every other PLY cron line, which is `curl -sf ... >/dev/null 2>&1` and fails silently. Secrets in `/root/.ply-cron.env` (chmod 600), not inline in the script. The digest posts even on a zero-signup day so silence means broken, not quiet. Webhook resolution: `DISCORD_WAITLIST_WEBHOOK` if set, else `DISCORD_WEBHOOK_URL`. **`DISCORD_WAITLIST_WEBHOOK` is now set on PROD only** (Coolify env, id `1539028929329893376` → Discord channel `signup`); staging is unset and has no digest cron, so it can't double-post. Rotating the channel = update that one Coolify env var + redeploy; no code change. Verified: 401 without/with wrong secret, 2-signup dump, zero-signup heartbeat, `?hours=168` backfill, and the failure-alert path (bad secret → exit 1 + Discord alert).

- **Organic-traffic instrumentation (2026-08-22, LIVE)**: PLY had PostHog and nothing else — no GA4, no Search Console, no `robots.txt`, no `sitemap.xml`, no `metadataBase`, no canonicals. The GA4 property/stream existed (`G-XL0JD7G416`, stream `15483086228`) but had **never received a hit** because the tag was never installed. Added: `GoogleAnalytics` from `@next/third-parties@16.1.6` in `layout.tsx`; `src/app/robots.ts` (allows the marketing surface, disallows `/api/`, `/admin`, `/dashboard`, `/onboarding`, `/calibrate`, `/feedback`, `/vouch/`, `/join/`); `src/app/sitemap.ts` (`/`, `/welcome`, `/faq`, `/thesis`, `/privacy`, `/terms`); `metadataBase` + per-page canonicals. **`/waitlist` now canonicalises to `/`** — the two served byte-identical `WaitlistCapture` pages, so Google had no signal which to keep; `/waitlist` stays live for the running Meta ads but is out of the sitemap.
- **Organic content cluster (2026-08-22)**: 22 pages shipped per `specs/location-seo-strategy.md` + `specs/organic-search-international.md` (Charles approved; overrides: in-language page for EVERY top-10 country now, Philadelphia added to wave 1). US: `/lds-singles` hub, 8 metro pages (SLC, Provo, Phoenix, Boise, DC, NYC, Rexburg, Philadelphia), 4 ranking pages (top-25 share, diaspora, all-869 reference w/ Dataset schema, global country ranking). International: 6 Spanish (`/es/solteros-sud/*` + index), 1 Portuguese (`/pt/namoro-sud/brasil`), 2 English (philippines, nigeria) — all with honest "not in your country yet" waitlist capture (phone+country, no ZIP): migration 023 `country` column, intl branch in `/api/waitlist` (E.164 8–15 digits; US path untouched), `IntlWaitlistForm`. Data: `scripts/build-seo-data.mjs` → `src/data/seo-*.json` from the committed CSVs + official Church country stats (31 Dec 2025). Footer gained a neutral-text "Cities" link (launch-plan §5: community not named on ad landing; anchor is neutral, target page is not). Content pages carry FAQPage JSON-LD; all in sitemap. Wave 2 metros (LA, Vegas, DFW, Seattle, Houston, Portland, Ogden, St. George) gated on wave-1 GSC impressions.
- **STILL OPEN (needs Charles)**: the Search Console **DNS TXT record is not added**. `people-like-you.com` is on Google Cloud DNS (`ns-cloud-b*.googledomains.com`) and there is no `gcloud` on this machine. Add TXT on the apex: `google-site-verification=vdfOCcwiWn_E21OIAGCCntoND3qg7zK7Wq6jC1XwzpI`. The same token is also emitted as a `<meta name="google-site-verification">` tag, which only verifies a URL-prefix property — a **Domain** property requires the DNS record.
- **The real organic constraint is not the plumbing**: `/` is a bare phone+ZIP form with no indexable body copy, and it is the URL that ranks. The three pages with actual content are `/welcome` (the old marketing homepage), `/faq`, and `/thesis`. Until `/` carries real copy or organic intent is deliberately routed to `/welcome`, Search Console will have almost nothing to work with.
...
- **2026-08-22 — Matching v2 APPROVED (D1–D10)**: decision memo (`specs/matching-v2-decision-memo.md`) → Charles picked: launch at 40–60 balanced; sequential same-candidate re-pitching (randomized K≤3 + angle order); 1 pitch/day; no fire quota; Battery-B quiz (~23 items) with quiz-driven voice story prompts; 4 angles × 2 leads randomized; NO own-pitch reveal (D7b); milieu sort term live day one behind `MILIEU_WEIGHT` flag with dual-score logging (D8b); pitch-taste Likert step; waitlist quiz phase 2. **Execution brief: `matching_algo-v2.md` (supersedes v1). Canonical test plan: `specs/matching-v2-test-plan.md` (T-SIM gate).** Questionnaire deep-dive charter: `specs/matching-v2-questionnaire-deep-dive.md` (V2-T0, dedicated session, blocks quiz/taste/prompt copy freeze). Build = V2-T0…T11, not yet started; everything else stays frozen.
- **2026-08-23 — V2-T0 questionnaire deep dive (IN PROGRESS, no code yet)**: research pass done and sourced (`specs/matching-v2-questionnaire-research.md` — killed the agree-disagree Big Five block; item-specific options beat agree-disagree on data quality and dodge acquiescence). Battery frozen at **23 items, ~3.5 min** after six inline passes with Charles (`specs/matching-v2-questionnaire-battery-v1.md`, rc6, voice = dry stems + warm-sincere framing). **Charles overruled charter §1 job 2 / criterion N3: quiz answers are HIDDEN DATA for the narrative, never pitch copy — closed-set answers aren't unique to anyone. Stories are the pitch material.** Politics is 3-way importance (hard filter on the top tier only, middle tier logged and unused). Q19 is text OR ≤30 s audio. Register now derives from two items, so `matching_algo-v2.md` §6.4 and test-plan U19 need rewriting; U17 is void. Voice-prompt map done (`specs/matching-v2-voice-prompt-map.md`, 47 prompts, coverage-based selection, reuses the live `PromptDef`). **Build farmed out**: `specs/farmout-v2-quiz-build.md` (V2-T1/T2/T4 → staging). Still owed by this workstream: D-QD3 (16 sample pitches), D-QD5 (UX flow), D-QD6 (pilot), D-QD7 (item analytics), D-QD8 (approval checklist), D-QD9 (waitlist variant).
- **2026-08-23 — V2-T1/T2/T4 BUILT (staging)**: the quiz is live in onboarding between basics and voice. Migration `024_matching_v2_quiz.sql` (`reader_traits`, `quiz_responses`, `voice_memos.prompt_source/prompt_seed`) — **note staging and prod share one Supabase, so 023 is applied to the production DB; it is additive and IF NOT EXISTS only.** New: `src/lib/quiz-battery.ts` (23 items, copy auto-derived from the spec), `src/lib/quiz-scoring.ts` (pure), `src/lib/voice-prompt-map.ts` (46 fished prompts + Q19 template + coverage selection), `src/components/QuizStep.tsx`, `/api/quiz`. `applyHardFilters` now applies the 3-way politics filter (`strong` filters at >2 steps bidirectionally; `prefer` logged only). Answers select up to 3 fished voice prompts; `voice_memos` records prompt provenance. vitest added (`npm test`, 132 tests) and wired into `deploy-staging.yml` as a pre-docker build gate. `matching_algo-v2.md` §6.4 and test-plan U16/U17/U19 rewritten; U23-U28 + a copy-freeze suite added. **Open for Charles**: the per-option trait values for the three nominal double-scored items (Q2/Q13/Q15) are arbitrary starting values, and the construct-budget table's claim that Q14 double-scores C is not supported by Q14's own definition — C ships with 2 indicators.
- **2026-08-24 — V2-T1/T2/T4 SHIPPED TO PROD, reconciled with the prompt picker.** Quiz is rc9: 22 items, no block-card interstitials, one unique emoji per option (rc8 had 89 emoji but only 83 distinct), Q22 stem names politics, and the nerd-out moved out of the quiz into the voice step. No trait double-scoring — an option set built so no answer lands worse than its siblings cannot also be scored ordinally on a desirable trait; **C = Q9 only and A = Q10 only, logged and never reported as measurements**; O (3) and E (2) carry the pre-registered hypotheses. **Branch collision resolved**: the picker (shipped to main 08-24) and the quiz rebuild both restructured `onboarding/page.tsx` (7 conflict regions). The picker won; `getPromptChoices` and `getProfileCompletion` now take a `personalised` set so fished prompts lead the picker's list and a recorded fished prompt actually advances angle coverage. 38 fished prompts + the nerd-out gained `short` labels (new copy for a new field; the frozen prompt text is untouched) — **Charles has not reviewed these 39 labels**. `PromptDef.exampleAnswer` is now optional. 134 tests green, incl. a copy-freeze suite asserting every stem/option/emoji byte-for-byte against the spec. **OWED: growth-cockpit pre-registration** (`ply.ga4.onboarding_completion`, direction down, 4-week window) — `MARKETING_BOT_API_KEY` is not in this project's env, on the server, or in repo secrets, so it could not be fired before shipping.
...
- [x] V2-T1: migration 023 + reader_traits/quiz_responses db helpers *(2026-08-23, staging)*
- [x] V2-T2: quiz step UI + scoring + reader_traits persist + 3-way politics hard filter *(2026-08-23, staging)*
- [x] V2-T4: voice-prompt map + dynamic prompt selection in the voice step *(2026-08-23, staging)*
- [ ] V2-T3: 16 sample pitches (D-QD3) + pitch-taste step — BLOCKED on D-QD3, which is unwritten
- [x] Pitch rationales v1 — migration 025 + same-call rationale/claims in `generateTrailer` + `savePitchRationale` at all 4 call sites + `/admin` Pitch Rationales tab *(2026-09-02; spec `specs/pitch-rationales.md`)*. Phase 2 (verifier) and bias covariates NOT built, deliberately. When the 16 V2-T3 sample pitches are written, each ships a hand-written `kind='sample'` rationale row via a seed script — authored by Claude, reviewed by Charles, never silent.
- [ ] Recurring R1: weekly metrics review (see §7)

- **Voice prompt bank v3 (2026-08-24, LIVE IN PROD)**: `QUESTION_BANK` replaced wholesale after a two-stage adversarial review of all 102 prompts (55 bank + 47 fished). Stage 1 (`specs/prompt-review-findings.md`) added two disqualifying criteria the old rules missed — **protagonist** (does a typical answer make the answerer the subject, or a witness?) and **retrievability** (superlatives and "the last time" of unindexed events are unanswerable) — and killed 6, including `laugh_hardest`, whose own shipped example was a story about a roommate. Stage 2 (`specs/prompt-set-v3.md`) re-graded everything against Charles's actual bar — *would the pitch this produces make a stranger want to meet the person, and is it fun?* — while keeping the structural criteria as a hard floor. Result: 35 kept, 59 lifted, 8 recast. Method was simulate-then-pitch: three modal spoken answers per prompt, then an attempt to write one pitch sentence under the §6.5 tone rules; a prompt that only yielded a *generic-passable* sentence (one writable about a thousand other people) failed even when structurally sound. **Live bank stays 55** (12/12/12/9/10) so `ONBOARDING_WEIGHTS` and `PromptDef` are unchanged; 12 ids retired and 12 added. Retired ids keep their text in `RETIRED_PROMPT_TEXT` and `getPromptText()` resolves live-or-retired, so historical `voice_memos` still render their question instead of a raw id. **The 47 fished prompts are spec-only** — `src/lib/voice-prompt-map.ts` does not exist yet, so V2-T4's dynamic selection is not actually wired in prod despite being checked off above; today every user gets 6 bank prompts from `getOnboardingPrompts`.
- **Voice step = prompt picker (2026-08-24, LIVE IN PROD)**: the fixed six-prompt carousel is replaced by a chooser. Every prompt gained a `short` label (<=8 words) so five options are scannable at once; the full two-clause question appears only after they pick, on the recording screen — the second clause is what produces a usable story, so the prompts themselves were **not** shortened. **Completeness is angle coverage, not a memo count**: `getProfileCompletion` marks a profile complete when all four pitch angles have >=1 story, derived from the generator's thin-data swap (an angle with no material is one we can never pitch that person from; six memos in one tier would leave three angles dead). `getPromptChoices` leads with uncovered angles, so the first screen spans all four and any pick advances the profile. The step is **skippable** per Charles (`canProceedVoice` was `recordings.size >= 2`, now unconditional), with incompleteness stated on the step and carried into ProfileTab. Also fixed: the old `handleSkipPrompt` called `getRandomPrompt`, which ignores tier and could replace someone's only comfort prompt with a second self-expansion one, defeating the coverage guarantee. **Bug caught only in a browser, not by types or the build**: `getPromptChoices` shuffles, and calling it during render made server and client draw different lists → React hydration mismatch → tree regenerated and component state lost. Choices are now drawn in an effect after mount with placeholder rows holding the layout. Verified in a real browser end-to-end: answering a *second* prompt in an already-covered tier correctly does not advance the meter.
- **BRANCH COLLISION (needs the quiz session)**: this change and the unreleased quiz rebuild on `staging` (rc8/rc9, V2-T2/T4) both restructure `src/app/onboarding/page.tsx`. Cherry-picking the picker onto `staging` produced **7 conflict regions**, so it was shipped to `main` only — `main` had no quiz work and applied cleanly. Whoever merges `staging` -> `main` must reconcile the voice step by hand; the picker is the newer design and the quiz's fished-prompt selection should feed `getPromptChoices` rather than the old carousel.
- **Pitch rationales — per-pitch provenance (2026-09-02)**: every generated pitch now writes one `pitch_rationales` row explaining why it was written — mechanical provenance logged by code (full prompt, model, exact inputs fed in, `inputs_omitted`, all 3 drafts + critic scores) plus the model's **same-call** self-report (`rationale` + a per-claim source map). Never a post-hoc "explain this pitch" call: that would confabulate a reasoning the model never had. Migration `025_pitch_rationales.sql` (new table, not columns on `daily_intros`, which already has migration drift). `generateTrailer` returns `provenance`; `savePitchRationale` (`src/lib/db.ts`) fires after `saveDailyIntro` at **all four** call sites (deliver-matches, feedback, cadence, voice-prompt-loop) and **never throws** — a rationale write failure is console.error + Sentry, never a lost intro. `max_tokens` 1024 → 2048; no other change to pitch content or generation logic. Admin tab **Pitch Rationales** (`/api/admin/pitch-rationales?limit=50`) renders each claim sentence-beside-source, with `source_type: none` (hallucination candidate) in red and `inference` in amber. **`inputs` records the fields both structured AND as rendered into the prompt** — the model quotes spans of a `values.join(', ')` line that no single array element contains, so a structure-only record made true claims look unsourced. Purpose is bias/hallucination/drift audit; **capture-only in v1, the verifier is phase 2** (`verified_at`/`verifier_model`/`flag_count` stay null). Bias covariates deliberately held (`covariates` reserved, unused). No retroactive rows for the 12 seed narratives or 8 Dane examples. Spec: `specs/pitch-rationales.md`. Harness: `npx tsx --env-file=.env.local scripts/verify-pitch-rationales.ts`.

- **STILL OPEN (needs Charles)**: the prompt rewrite is a falsifiable bet on story quality and should have been **pre-registered with the growth cockpit before shipping**, but `MARKETING_BOT_API_KEY` is not present in this project's environment (not in shell env, not in any `.env`). Registration payload is ready — see the handoff — and key distribution to PLY is the known gap.
