...
- **2026-09-02 — Voice prompts v4 (session branch, NOT yet on staging)**: Charles reviewed v3 on prod: prompts "kinda wack", the fished set on prod was still the pre-review copy (incl. the killed "Who taught you to be on time?" and four superlatives), the mandatory nerd-out was a bare category question ("What do you nerd out on?"), and `extraction.ts` built its prompt lookup from the bank only so every fished memo told Haiku the person answered "fished_Q11_4". Decisions: **copy now, before testing; voice = plain and warm, a wink only in `fun`; prompts must work for men (action over state, the guy test); bank stays ~100, prune by ignore-rate never by taste; example on/off split by account; Charles records with wife, brother, friends.** Shipped on `session/s-0902-2105`: (1) `resolvePromptText` in voice-prompt-map, used by extraction + ProfileTab; (2) migration 025 + `prompt_events` + `/api/prompt-event` + `exampleArm`/`logPromptEvent` (`src/lib/prompt-events.ts`): picker shown/picked/passed, skipped-after-open, seconds-to-first-take, re-record count, example arm, all on the memo row and in `prompt_metrics`; (3) **`specs/prompt-set-v4.md` = the frozen copy source** for 60 bank (13/13/13/11/10) + 38 fished + the two-clause nerd-out, asserted byte-for-byte by the copy-freeze suite (bank now frozen too). `obsession` retired. Six new bank ids. Fished reconciled to the rc11 quiz (Q4:3, Q9:3, Q11:2 written fresh). **Awaiting Charles's read of the full set before staging.** Test protocol: one split (example on/off), measure seconds + words per memo vs the March baseline (median 18 s / 26 words), seconds-to-record and re-records as the stall signal, extraction depth, hand-labelled protagonist and example-copying, then a pitch per person read in pairs by a stranger. Test accounts land in the shared prod DB: delete them after the read.
