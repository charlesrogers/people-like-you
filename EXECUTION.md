...
- **STILL OPEN (needs Charles)**: (a) **credential rotation** — attacker had root in a container on the `coolify` network for 4½ months; `SUPABASE_SERVICE_ROLE_KEY`/`DATABASE_URL` should be considered exposed; (b) decide whether sales-analyzer is rebuilt on a patched Next.js or deleted outright; (c) **monitoring gap** — Uptime Kuma did not surface a 5-day outage; (d) memory limits are runtime-only and will be lost on next Coolify redeploy — set them in the Coolify UI.
...
- [~] T16c: waitlist → **phone + ZIP only** + success popup (city + position + copy-paste invite) + root `/` serves the waitlist, marketing page → `/welcome`. Migration 022. *(2026-08-13, built + API verified against prod DB; awaiting Charles's visual sign-off on staging before main)*
