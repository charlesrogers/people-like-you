...
- **CRON INCIDENT (resolved 7/16, follow-up open)**: two sessions converged on the same signal. Final diagnosis (other session, commit 3ad51ed): PLY crons were split-brain — the real ones run **server-side** (`/etc/cron.d/coolify-apps`) with the correct secret; the GH Actions `crons.yml` was a drifted duplicate (rotate-cron-secret.sh updates Coolify + server cron, never the GH secret) that 401'd on every run. Fix: `crons.yml` deleted; `second-date-check` (which existed ONLY in GH Actions and had never run) added server-side and verified 200. No GH CRON_SECRET rotation needed. **Follow-up open — delivery still not proven**: (a) `daily_intros` has no rows since 2026-03-30. Checked 7/16 evening: NOBODY is paused (0 paused / 11 active in `user_cadence`), but only 11 of 23 users have a cadence row, ALL with `delivery_hour=11` UTC (=4–5am MT — the T22 bad default), and **the prod container logged zero cron hits in the last 24h** (deliver-matches logs "Cron: N users eligible" on every authed call — nothing). So the server-side cron is either not calling prod deliver-matches or still 401ing there, despite 3ad51ed's verification of second-date-check. To verify from the server: `grep deliver /etc/cron.d/coolify-apps`, then curl the endpoint with the secret from that file and watch `docker logs` for the "Cron:" line + a new `daily_intros` row. (b) missing cadence rows for 12 users need backfill (the auto-create was a one-time migration; signup path may not create them — check). (c) server-side crons need a failure alert path (Discord) — silent-cron rule.
