...
### 2026-09-10 — Real onboarding must preserve work and explain choices
Charles's phone walkthrough found an undisclosed 90-second recording cutoff, a photo-upload gate, hover-only photo removal, unclear one-per-group story selection, and an intimidating full step list. Remove the automatic cutoff, keep uploaded recordings intact, make photos skippable even after an upload error, expose touch-sized removal, show full prompt text and group purpose before selection, and say explicitly that four answers means one per group. Never require failure/embarrassment as the universal shape of a story. Propagate actual upload errors instead of replacing every cause with “Failed to upload photo.”
