...
- [ ] P0c Instrument the voice step before Charles records: picker impression, pick, skip-after-open, seconds from prompt open to record, re-record count. PostHog `track` events + columns on `prompt_metrics`.
- [ ] P1a Nerd-out gets its second clause (plain): pending Charles's OK on wording.
- [ ] P1b Fished 38 → v3 structure in plain voice, reconciled to the CURRENT quiz options (Q4:3, Q9:3, Q11:2 have no v3 text; write new). Update `specs/matching-v2-voice-prompt-map.md` and the copy-freeze suite together.
- [ ] P1c Bank → plain voice. Fold collapsed constructs (rabbit_hole/obsession vs nerd-out; taught_yourself/figured_it_out; hardest_thing/superpower/mentor_moment). Re-aim comfort at warmth. Grow to ~100 total with construct-family caps (witness device ≤15%). Examples rewritten as a person talking, two sentences, no button. Short labels reviewed.
- [ ] P1d Full set to Charles for review before it ships.
- [ ] P2 Test protocol written (who, n, split, what's measured, how the pitches are read).
